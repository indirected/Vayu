...
public class ShutGunPod : MonoBehaviour
{
    ...
    {
        Debug.Log("collision on ShutGun pod");
        try
        {
            ...
            if (other.tag == "Player")
            {
                ...
                var RequiredLevel = other.GetComponent<WeaponPickingSystem>().ShutGunLevel;
                if (PlayerLevel >= RequiredLevel)
                {
                    other.GetComponent<WeaponPickingSystem>().ActivateShutGun();
                    PodControllerObj.GetComponent<GunPodSpawnControll>().CurrentShutGuns--;
